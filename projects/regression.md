...
\\[R^2_{\text{McFadden}} = 1 - \frac{L_{\text{ur}}}{L_{0}}  \\]
...
\
Notice that the numerator and denominator functions are log likelihood functions. The log likelihood of function gives the likelihood of observing a sample with given function parameters. In McFadden's $R^2$, the numerator and denominator are log likelihoods with coefficients $\hat{\theta}$ and $\hat{\theta}_{\text{0}}$ which maximize the likelihood of observing the given data. The difference between the two is that $\hat{\theta}$ represents the full vector of coefficients while $\hat{\theta}_{\text{0}}$ represents only the intercept term (first coefficient) while setting the rest of the coefficients to zero. \
\
Note that since $G$ is between $0$ and $1$, and the log of a number less than $1$ is negative, that both log-likelihoods are negative. If the $x$'s  has no predictive power, then the log likelihood will be the same. So: \\[ L_{\text{ur}} = L_{0} \implies R^2_{\text{McFadden}} = 0 \\]
